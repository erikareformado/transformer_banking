Module open_delta_lines
    Public Class open_delta
        ...
        Public Function select_open_delta_points(transformer)
            Dim result As New List(Of String)

            query = "select * from open_delta_lines where transformer_details_id = '" & transformer & "' order by id asc"
            Dim da As New Odbc.OdbcDataAdapter(query, conn)
            Dim dt As New DataTable
            da.Fill(dt)
        ...
        Public Function select_count_open_delta_points(transformer)
        ...
        Public Sub delete_unwanted_connection()
            query = "DELETE FROM open_delta_lines WHERE id in (
                SELECT id FROM open_delta_lines ORDER BY id desc LIMIT 1)"
            Dim da As New Odbc.OdbcDataAdapter(query, conn)
            Dim dt As New DataTable
            da.Fill(dt)
        End Sub
        ...
        Public Function select_clamp_count()
            ...
            query = "select * from open_delta_lines where clamp_meter = '1' order by id asc"
            Dim da As New Odbc.OdbcDataAdapter(query, conn)
            Dim dt As New DataTable
            da.Fill(dt)
